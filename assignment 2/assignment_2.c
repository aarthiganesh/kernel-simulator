...
struct process{
    ...
    int mem;
	int waittime;
	int turnaround; //keeps track of the turnaround time
	int starttime; //used to store the starting time of a process
	int stoptime; //used to store the time a process terminates
};
...
//verify if queue if full
int isFull(struct Queue* queue) 
{ 
    return (queue->size == queue->capacity); 
} 
//verify if queue is emoty
...
//gets the front element of a queue
...
//gets the rear element of a queue
    ...
    if (strcmp(argv[3],"fcfs")==0){
        ...
        while(1){
            ...
            for(i=0;i<numCommands;i++)
            {
				...
				//increments cpuCounter when process is in RUNNING state
			...
			//changing from READY to RUNNING 
