...
int main(int argc,char *argv[])
{
	...
	char command[20] = "fcfs";
    fptr = fopen("input2.txt", "r");
	outfile = fopen("output2.txt", "w"); //create and open output.txt file to write
...
// PRIORITY SCHEDULING
    if(strcmp(command,"priority")==0){//to store the processes that are ready
...
// MEMORY MANAGEMENT
    if(strcmp(command,"mm1")==0 || strcmp(argv[3],"mm2")==0){
        int totalMEM = 0;
        int availableMEM = 0;
        int usableMEM = 0;
        int usedMEM = 0;
